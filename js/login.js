...
if (isLocalStorageSupported()) {
  ...
  const senhaValida = /^(?=.*[A-Z])(?=.*[!#@$%&])(?=.*[0-9])(?=.*[a-z]).{6,15}$/.test(usuario.password);
...
function validate() {
    ...
    const cpfNumerico = cpf.replace(/[^\d]/g, ''); // Remova caracteres não numéricos do CPF
    ...
    if (
      nome === '' ||
      data === '' ||
      !genderPattern.test(gender) || // Valida o campo "gender" com a expressão regular
      mae === '' ||
      cpfNumerico === '' ||
      celular === '' ||
      telefone === '' ||
      ...
      showToast2('O campo "login" só permite letras.');
